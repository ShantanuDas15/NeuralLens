...
from models.database import EnhancementJob, ModelConfig, User
...
async def test_empty_history(client, mock_verify_token):
    ...
    response = client.get(
        "/api/history", headers={"Authorization": "Bearer fake-token"}
    )
...
async def test_history_pagination_and_soft_delete(client, mock_verify_token):
    ...
    # Get page 1, size 2
    response = client.get(
        "/api/history?page=1&page_size=2",
    ...
    response2 = client.get(
        "/api/history?page=2&page_size=2",
        headers={"Authorization": "Bearer fake-token"},
    )
    ...
    response3 = client.get(
        "/api/history?page=1&page_size=100",
        headers={"Authorization": "Bearer fake-token"},
    )
